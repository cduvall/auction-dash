import type { LotStats } from "@/shared/types";
import { fmt, pct } from "@/shared/lib/format";
import { StatCard, Icons } from "@/features/dashboard/components/StatCard";
...
export function StatsGrid({ stats, onUntouchedClick }: StatsGridProps) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-2 sm:gap-3 mb-6">
      ...
      {stats.totalMedianValue > 0 && <StatCard label="Total Median Value" value={fmt(stats.totalMedianValue)} sub="Sum of all median estimates" icon={Icons.dollar} color="#909194" />}
      <StatCard label="Total High Bids" value={fmt(stats.totalHighBids)} sub="Sum of all current bids" icon={Icons.trendUp} color="#cc7722" />
      {stats.totalMedianValue > 0 && <StatCard label="Value Gap" value={fmt(stats.gap)} sub="Median value minus bids" icon={Icons.gap} color="#6b705c" />}
